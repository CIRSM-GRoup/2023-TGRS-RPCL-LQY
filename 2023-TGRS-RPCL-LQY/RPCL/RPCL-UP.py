...
parser.add_argument("-s","--shot_num_per_class",type = int, default = 2)
...
def run_kmeans(x, Num_cluster):
    ...
    clus.niter = 20
    clus.nredo = 5
    clus.max_points_per_centroid = 25
    ...
    a = results['centroids']
    pro = torch.tensor([item.cpu().detach().numpy() for item in a]).cuda()
    pro = torch.squeeze(pro)
    return pro
...
for iDataSet in range(nDataSet):
    ...
    for episode in range(3001):  # EPISODE = 90000
        ...
        if episode < 1000:
            ...
            supports_1 = supports[0:18:2, :, :, :]
            supports_2 = supports[1:18:2, :, :, :]
            # calculate features
            ...
            labels_proto = torch.linspace(0, CLASS_NUM-1, steps=9).long().cuda()
            query_proto = query_features.reshape(CLASS_NUM, QUERY_NUM_PER_CLASS, -1).mean(dim=1)  # 16, 160
            # Prototype network
            ...
            # pr_loss
            logits_f1 = euclidean_metric(support_features_1, support_proto)
            logits_f2 = euclidean_metric(support_features_2, support_proto)
            self_loss = crossEntropy(logits_f1, labels_proto) + crossEntropy(logits_f2, labels_proto)
            self_loss = self_loss / (2 * CLASS_NUM)

            # cl_loss
            cl_loss = infoNCE_Loss(support_features_1, support_features_2)
            # fsl_loss
            ...
            if episode > 0:
                #proto_cluster = run_kmeans(query_features, CLASS_NUM) #16 * 160
                logits_pro_1 = euclidean_metric(support_features_1, query_proto)
                logits_pro_2 = euclidean_metric(support_features_2, query_proto)
                proto_loss = crossEntropy(logits_pro_1, labels_proto) + crossEntropy(logits_pro_2, labels_proto)
                proto_loss = proto_loss / (2 * CLASS_NUM)
                loss = cl_loss + proto_loss + self_loss + f_loss
            else:

            # total_loss = fsl_loss + domain_loss
                loss = domain_loss + cl_loss  # 0.01
        ...
        # target domain few-shot + domain adaptation
            ...
            labels_proto = torch.linspace(0, CLASS_NUM - 1, steps=9).long().cuda()
            query_proto = query_features.reshape(CLASS_NUM, QUERY_NUM_PER_CLASS, -1).mean(dim=1)
            # Prototype network
            if SHOT_NUM_PER_CLASS > 1:
                support_proto = support_features.reshape(CLASS_NUM, SHOT_NUM_PER_CLASS, -1).mean(dim=1)  # (9, 160)
            else:
        ...
        if (episode + 1) % 100 == 0:  # display
            ...
            print('episode {:>3d}:  domain loss: {:6.4f}, proto loss: {:6.4f} , contrastive loss: {:6.4f} , acc {:6.4f}, loss: {:6.4f}'.format(episode + 1, \
                                                                                                                domain_loss.item(),
                                                                                                                proto_loss.item(),
                                                                                                                cl_loss.item(),
...
utils.classification_map(hsi_pic[4:-4, 4:-4, :], best_G[4:-4, 4:-4], 24,
                         "classificationMap/UP_{}shot.png".format(TEST_LSAMPLE_NUM_PER_CLASS))
